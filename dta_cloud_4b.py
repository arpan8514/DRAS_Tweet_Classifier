...
@app.route("/")
def create_file():
    tweet_key = sys.argv[1]
    tweet_dump_file = "output_got.txt"
    if (os.path.isfile(tweet_dump_file)):
        os.remove(tweet_dump_file)

    day_1 =  datetime.date.fromordinal(datetime.date.today().toordinal())
    day_2 = datetime.date.fromordinal(datetime.date.today().toordinal()-5)

    os.system('python Exporter.py --querysearch {} --since {} --until {} \
               --language lang:en'.format(tweet_key, day_2, day_1))
